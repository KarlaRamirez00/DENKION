...
def login_view(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            if user.is_staff:
                # Si es personal administrativo, redirige a la página deseada
                return redirect("crud_clientes")
            
            # Si no es personal administrativo, agrega el usuario al contexto
            # y renderiza la plantilla clientes_login.html
            return render(request, "clientes/clientes_login.html", {'user': user})
        else:
            # Usuario no autenticado, puedes manejar esto de acuerdo a tus necesidades
            return render(request, "clientes/clientes_login.html", {'login_error': True})
